...
class User:
    def __init__(self, name, email, user_accounts = 1):
        self.name = name
        self.email = email
        self.accounts = {}
        for account in range(user_accounts):
            self.account[account +3200] = (BankAccount(int_rate=.07, balance = 0))
        print(str(len(self.account))+'account created:')
        for key, account in self.account.items()
            print('account number:', key, account)
